	...
	for i < len(query) {
		...
		if isIdentStart(c) {
			...
			if strings.EqualFold(word, "CAST") || strings.EqualFold(word, "SAFE_CAST") {
				if open, lit, end, ok := naiveTimestampCastLiteral(query, i); ok {
					b.WriteString(query[i:open])
...
// naiveTimestampCastLiteral matches `( '<naive>' AS TIMESTAMP )` at query[i:]
// and returns the offsets of the literal's opening quote (open == lit) and
// of the byte after its closing quote.
func naiveTimestampCastLiteral(query string, i int) (open, lit, end int, ok bool) {
	skip := func(k int) int {
		for k < len(query) && (query[k] == ' ' || query[k] == '\t' || query[k] == '\n' || query[k] == '\r') {
			k++
		}
