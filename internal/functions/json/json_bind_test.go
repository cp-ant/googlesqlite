...
func TestJsonSubscriptByRuntimeKey(t *testing.T) {
	...
	doc := value.JsonValue(`{"Fn::If": 1, "a.b": {"x": [7]}, "arr": [1, {"k": 2}]}`)
	for _, tc := range []struct {
		field value.Value
		want  string
	}{
		{value.StringValue("Fn::If"), "1"},
...
func TestBindJsonKeys(t *testing.T) {
	...
	// Only keys containing '.' or '"' are quoted.
	got, err = BindJsonKeys(value.StringValue(`{"a b":1, "s3:prefix":2, "a.b":{"c":3}}`))
	if err != nil {
		t.Fatal(err)
	}
	arr = mustArray(t, got)
	if fmt.Sprint(arr.Values) != `["a.b" "a.b".c a b s3:prefix]` {
		t.Fatalf("got %v", arr.Values)
	}
