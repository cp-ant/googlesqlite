...
// navigationWindowNative backs FIRST_VALUE / LAST_VALUE / NTH_VALUE with
// IGNORE NULLS, which SQLite's built-ins lack: pick selects from the
// non-NULL values of the active frame.
...
// extremumWindowNative is MIN / MAX over values whose encoded form SQLite
// cannot order itself (DATETIME, TIME, STRING, NUMERIC, ...).
